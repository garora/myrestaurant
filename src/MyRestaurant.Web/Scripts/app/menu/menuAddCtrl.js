(function (app) {
    ...
    menuAddCtrl.$inject = ['$scope', '$location', '$routeParams', 'apiService', 'notificationService', 'fileUploadService'];

    function menuAddCtrl($scope, $location, $routeParams, apiService, notificationService, fileUploadService) {
        ...
        function AddMovieModel() {
            apiService.post('/api/menu/add', $scope.movie,
            addMovieSucceded,
            addMovieFailed);
        }
        ...
        function addMovieSucceded(response) {
            notificationService.displaySuccess($scope.movie.Title + ' has been submitted to my Restaurant');
